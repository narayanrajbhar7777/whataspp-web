...
app.post('/api/upload-and-schedule-whatsapp', uploadScheduled.array('documents', CONFIG.MAX_FILES_PER_SCHEDULE), (req, res) => {
    ...
    if (!check.success || validatePhoneLimit(list)) { 
        files.forEach(f => deleteFileSafe(f.path)); 
        return res.status(400).json({ success: false, error: check.error || 'Bad validation parameters.' }); 
    }
...
app.post('/api/schedule-location-report', (req, res) => {
    ...
    const check = mathService.calculateFirstScheduleDate({ scheduleMode: mode, scheduleAt, startDate, time, weeklyDay, monthlyDay });
    if (!check.success || validatePhoneLimit(list) || !folderPath) return res.status(400).json({ success: false, error: check.error || 'Missing directory configuration references.' });
...
app.get('/api/schedules', (req, res) => { res.json({ success: true, schedules: loadSchedules() }); });
app.delete('/api/schedules/:id', (req, res) => { saveSchedules(loadSchedules().filter(s => s.id !== req.params.id)); res.json({ success: true, message: 'Schedule entry purged.' }); });
